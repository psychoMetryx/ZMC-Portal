...
EduZMC adalah aplikasi **single-page** berbasis React, TypeScript, dan Vite yang menyajikan artikel edukasi pasien, alat interaktif ringan, serta tombol kontak cepat untuk fasilitas Zihan Medical Center (ZMC). Seluruh fitur berjalan di browser tanpa autentikasi; data untuk alat interaktif disimpan di `localStorage` sehingga pasien dapat menggunakan portal secara anonim.
...
| Model deploy | SPA statis, tanpa backend | Tidak ada pemanggilan API di kode | 
| Bahasa utama | Bahasa Indonesia | Teks UI & artikel | 
| Autentikasi | Tidak ada (akses anonim) | Seluruh state lokal | 

## Fitur Utama
- **Router sederhana berbasis state.** `App.tsx` hanya mengenal dua keadaan (`home` atau `articleId`) dengan tombol breadcrumb dan kembali untuk berpindah antar tampilan.【F:App.tsx†L7-L49】
- **Katalog artikel tunggal.** Semua konten (metadata dan komponen React) didefinisikan di `services/articleRegistry.tsx`, lalu dirender sebagai kartu beranda melalui `Home.tsx`. Filter kategori memakai enum `Category` dari `types.ts`.【F:services/articleRegistry.tsx†L19-L121】【F:pages/Home.tsx†L14-L61】【F:types.ts†L3-L18】
...
| Routing | State sederhana `currentView` (tidak memakai React Router) | `App.tsx` | 
...
3. `App.tsx` menentukan tampilan aktif berdasarkan state `currentView`, menemukan artikel yang cocok di registri, dan membungkus semua halaman dengan `Layout` agar CTA dan navigasi konsisten.【F:App.tsx†L7-L49】
4. `Home.tsx` membuat daftar kategori dari enum `Category`, menerapkan filter, dan menampilkan kartu artikel untuk setiap entri registri.【F:pages/Home.tsx†L14-L61】
5. `Layout.tsx` menyediakan header, menu mobile, footer, dan tombol WhatsApp mengambang sehingga kontak utama tetap tersedia di setiap halaman.【F:components/Layout.tsx†L15-L247】
...
3. **Registrasi**: tambahkan entri ke array `ARTICLES` di `services/articleRegistry.tsx` dengan `id`, `title`, `description`, `icon` FontAwesome, `category`, serta `component` yang merender artikel baru.【F:services/articleRegistry.tsx†L19-L121】
4. **Uji navigasi**: jalankan `npm run dev`, pastikan kartu beranda muncul, breadcrumb dan tombol kembali berfungsi, serta CTA WhatsApp di header/footer masih terlihat.【F:App.tsx†L19-L48】【F:components/Layout.tsx†L15-L247】
