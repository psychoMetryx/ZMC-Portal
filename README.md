...
## Fitur Utama
- **Router multi-halaman dengan deep-link.** `App.tsx` memakai React Router untuk rute `/` (beranda) dan `/artikel/:id` (artikel). Navigasi kartu artikel memanggil `navigate('/artikel/<slug>')`, breadcrumb kembali ke `Home`, dan direct-access URL akan memuat artikel yang sama tanpa state sebelumnya.【F:App.tsx†L6-L48】【F:pages/ArticlePage.tsx†L9-L58】
